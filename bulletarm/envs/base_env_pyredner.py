...
from bulletarm.pybullet.utils.sensor import Sensor, SensorPyRedner
...
def version(ver):
    def decorator(func):
        func.__version__ = ver
        return func
    return decorator


class BaseEnvPyRedner(object):
  '''
  Base Env Class.
  ...
  def __init__(self, config, dtype=None, device=None):
    ...
    self.object_init_metadata = None
  ...
  def _getObservation(self, meshes, action=None):
    ...
    self.heightmap = self._getHeightmap(meshes)
  ...
  def getInHandImage(self, heightmap, x, y, z, rot, next_heightmap):
    """
    PyTorch version of getInHandImage
    
    Args:
        heightmap: torch.Tensor or numpy array
        x, y, z: float coordinates
        rot: tuple of rotation angles (rx, ry, rz)
        next_heightmap: torch.Tensor or numpy array
        device: torch device
    """
    (rx, ry, rz) = rot
    ...
    if isinstance(heightmap, np.ndarray):
        heightmap = torch.from_numpy(heightmap).float().to(self.device)
    if isinstance(next_heightmap, np.ndarray):
        next_heightmap = torch.from_numpy(next_heightmap).float().to(self.device)
    
    # Pad heightmaps for grasps near the edges of the workspace
    pad_size = int(self.in_hand_size / 2)
    heightmap = F.pad(heightmap, (pad_size, pad_size, pad_size, pad_size), 'constant', 0.0)
    next_heightmap = F.pad(next_heightmap, (pad_size, pad_size, pad_size, pad_size), 'constant', 0.0)
    ...
    if self.in_hand_mode.find('proj') > -1:
        ...
        # Convert to numpy for skimage rotation, then back to torch
        crop_np = crop.cpu().numpy()
        crop_np = sk_transform.rotate(crop_np, np.rad2deg(-rz))
        crop = torch.from_numpy(crop_np).float().to(self.device)
        return crop.reshape((1, self.in_hand_size, self.in_hand_size))

  @version("pyredner")
  def getInHandOccupancyGridProj(self, crop, z, rot):
    """
    PyTorch version of getInHandOccupancyGridProj
    ...
    # Ensure crop is on the correct device
    ...
    # Round crop values
    crop = torch.round(crop * 1e5) / 1e5  # equivalent to np.round(crop, 5)
    size = self.in_hand_size

    # Create z-levels tensor
    z_offsets = torch.arange(size, device=self.device, dtype=torch.float32) - size/2
    zs = z + z_offsets * self.heightmap_resolution
    zs = zs.view(1, 1, -1).expand(size, size, -1)
    
    # Expand crop to match zs dimensions
    c = crop.unsqueeze(-1).expand(-1, -1, size)
    
    # Create occupancy grid
    ori_occupancy = c > zs

    # Get point coordinates where occupancy is True
    point_indices = torch.nonzero(ori_occupancy, as_tuple=False)
    if point_indices.shape[0] == 0:
        return torch.zeros((3, size, size), device=self.device)

    # Center the points
    ori_point = point_indices.float() - size/2
    ...
    for i in range(int(np.ceil(ori_point.shape[0] / batch_size))):
        ...
        end_idx = min(batch_size * (i + 1), ori_point.shape[0])
        batch_points = ori_point[start_idx:end_idx]
        transformed_batch = torch.matmul(R_tensor, batch_points.T)
        transformed_points.append(transformed_batch)
    
    if transformed_points:
        point = torch.cat(transformed_points, dim=1)
    else:
        return torch.zeros((3, size, size), device=self.device)
    
    # Re-center and round points
    point = point + size/2
    point = torch.round(point).long()
    
    # Filter valid points
    valid_mask = ((point >= 0) & (point < size)).all(dim=0)
    ...
    occupancy = torch.zeros((size, size, size), device=self.device)
    occupancy[point[0], point[1], point[2]] = 1
    
    # Apply median filter (using scipy for now, could be replaced with torch implementation)
    occupancy_np = occupancy.cpu().numpy()
    occupancy_np = median_filter(occupancy_np, size=2)
    occupancy = torch.from_numpy(occupancy_np).to(self.device)
    occupancy = torch.ceil(occupancy)

    # Create projections along each axis
    projection = torch.stack([
        occupancy.sum(dim=0),  # sum along x-axis
        occupancy.sum(dim=1),  # sum along y-axis
        occupancy.sum(dim=2)   # sum along z-axis
    ])
    
    return projection

  @version("pyredner")
  def torch_median_filter_3d(input_tensor, kernel_size=3):
      """
      A simple 3D median filter implementation using PyTorch
      Note: This is a basic implementation and may not be as optimized as scipy's version
      """
      device = input_tensor.device
      pad_size = kernel_size // 2
      
      # Pad the input
      padded = F.pad(input_tensor, [pad_size] * 6, mode='constant', value=0)
      ...
      for i in range(d):
          for j in range(h):
              for k in range(w):
                  ...
                  neighborhood = padded[i:i+kernel_size, j:j+kernel_size, k:k+kernel_size]
                  # Compute median
                  output[i, j, k] = torch.median(neighborhood.flatten())
  ...
  def getEmptyInHand(self):
    if self.in_hand_mode.find('proj') > -1:
        return torch.zeros((3, self.in_hand_size, self.in_hand_size), 
                          device=self.device, dtype=self.dtype)
    else:
        return torch.zeros((1, self.in_hand_size, self.in_hand_size), 
                          device=self.device, dtype=self.dtype)
